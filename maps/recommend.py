...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        t = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(i) for i in t]
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    d = {restaurant_name(restaurant): user_rating(user, restaurant_name(restaurant)) for restaurant in reviewed}
    not_reviewed = [restaurant for restaurant in restaurants if restaurant not in reviewed]
    d.update({restaurant_name(restaurant): predictor(restaurant) for restaurant in not_reviewed})
    return d
