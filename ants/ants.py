...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def look_for_bees(place2, i):
            if place2 == hive:
                return None
            elif place2.bees and i >= self.min_range and i <= self.max_range:
                return random_or_none(place2.bees)
            else:
                return look_for_bees(place2.entrance, i + 1)
        return look_for_bees(self.place, 0)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for element in copy_list:
            element.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):  
    ...
    def __init__(self, armor=1):
        super().__init__(armor)
        if QueenAnt.exists_queen:
        	self.true_queen = False
        	self.damage = 0
        else: 
        	QueenAnt.exists_queen = True
            	...
            	if place.ant:
            		...
            		if place.ant.is_container:
            			if place.ant.contained_ant:
	            			if place.ant.contained_ant.double_damage == False:
	            				place.ant.contained_ant.damage *= 2
	            				place.ant.contained_ant.double_damage = True


            	return look_for_bees(place.exit)
        
        look_for_bees(self.place.exit)
...
def apply_effect(effect, bee, duration):
    ...
    changed_action = effect(bee.action, bee)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            apply_effect(make_scare, target, 3)
