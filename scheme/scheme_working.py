...
def eval_all(expressions, env):
    ...
    #if do_begin_form(expressions, env):
    #else:
...
def make_let_frame(bindings, env):
    ...
    return env.make_child_frame(symbols, expressions)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        symbol = target.first
        exps = Pair(target.second, expressions.second)
        func = MacroProcedure(target.second, expressions.second, env)
        func.apply_macro(symbol, env)
        return symbol
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
