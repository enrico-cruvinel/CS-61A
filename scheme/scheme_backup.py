...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            operand = rest
            return scheme_eval(operator.apply_macro(operand, env), env)
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        aux = expressions
        while aux.second is not nil:
            scheme_eval(aux.first, env)
            aux = aux.second
    return scheme_eval(aux.first, env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('num of argument values does not match num of formal parameters')
        else:
            new_frame = Frame(self)
            d = dict()
            while formals or vals is not nil:
                d[formals.first] = vals.first
                vals, formals = vals.second, formals.second
            new_frame.bindings = d
            return new_frame
        # END PROBLEM 11
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong num of parameters passed')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        exps = Pair(target.second, expressions.second)
        func = do_lambda_form(exps, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    #self.env.make_child_frame(self.formals, args)
    
    # check_form(bindings.first, 2, 2)
    # formals = Pair(bindings.first.first, nil)
    # args2 = Pair(scheme_eval(bindings.first.second.first, env), nil)
        # formals.first, formals.second = bindings.first.first, formals
        # args2.first, args2.second = scheme_eval(bindings.first.second.first, env), args2

    formals = bindings.map(lambda expr: expr.first)
    check_formals(formals)
    
    args = bindings.map(lambda expr: scheme_eval(expr.second.first, env))
